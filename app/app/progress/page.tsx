...
type TopicProgress = { course: string; topic: string; total_questions: number; attempted_questions: number; correct_questions: number; accuracy: number; coverage: number };
type ProgressData = { sessions: PracticeSession[]; sprints: SprintSession[]; courses: CourseProgress[]; topics: TopicProgress[]; coverage: { questions: number; answered: number; topics: number; topicsCovered: number; percentage: number } };
...
export default function ProgressPage() {
  ...
  useEffect(() => { void refresh().catch(() => setProgress({ sessions: [], sprints: [], courses: [], topics: [], coverage: { questions: 0, answered: 0, topics: 0, topicsCovered: 0, percentage: 0 } })); void fetch("/api/cards").then((response) => response.json()).then((data) => setRuleCards(data.stats ?? { reviewed: 0, due: 0, passed: 0 })).catch(() => undefined); }, []);
  ...
  return <main className="progress-page">
    ...
    {progress === null ? <p className="muted" style={{ marginTop: 28 }}>Loading your sessions…</p> : <>
      ...
      <section className="panel topic-progress"><p className="eyebrow">Progress by topic</p>{progress.topics.length ? progress.topics.map((topic) => <article className="accuracy-row" key={`${topic.course}-${topic.topic}`}><div><strong>{topic.topic}</strong><span>{courseNames[topic.course]} · {topic.attempted_questions}/{topic.total_questions} questions</span></div><div className={`bar ${accuracyTone(topic.accuracy)}`}><span style={{ width: `${topic.accuracy}%` }} /></div><b>{topic.attempted_questions ? `${topic.accuracy}%` : "—"}</b></article>) : <p className="muted">Topics will appear as questions are assigned by the review team.</p>}</section></>}
