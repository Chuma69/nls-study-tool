...
export default function ProgressPage() {
  ...
  return <main className="progress-page">
    ...
    {progress === null ? <p className="muted" style={{ marginTop: 28 }}>Loading your sessions…</p> : <>
      <section className="progress-stat-grid"><article className="panel progress-stat"><p className="eyebrow">Overall accuracy</p><strong>{overallAccuracy}%</strong><span>{answered ? `${correct}/${answered} questions correct` : "Answer a question to begin"}</span></article><article className="panel progress-stat"><p className="eyebrow">Questions done</p><strong>{answered}</strong><span>{progress.coverage.percentage}% of the available bank covered</span></article><article className="panel progress-stat"><p className="eyebrow">Rule cards</p><strong>{ruleCards.reviewed}</strong><span>{ruleCards.due ? `${ruleCards.due} due for review` : "No cards due right now"}</span></article><article className="panel progress-stat"><p className="eyebrow">Avg pace</p><strong>{averagePace ? `${averagePace}s` : "—"}</strong><span>target is 60s a question</span></article></section>
      <div className="progress-detail-grid"><section className="panel accuracy-panel"><p className="eyebrow">Accuracy by course</p>{progress.courses.map((course) => <article className="accuracy-row" key={course.course}><div><strong>{courseNames[course.course] ?? course.course}</strong><span>{course.attempted_questions}/{course.total_questions} questions · {course.covered_topics}/{course.total_topics} topics</span>{course.attempted_questions > 0 && <button className="clear-course" type="button" onClick={() => { void clearCourse(course.course); }}>Clear progress</button>}</div><div className={`bar ${accuracyTone(course.accuracy)}`}><span style={{ width: `${course.accuracy}%` }} /></div><b>{course.attempted_questions ? `${course.accuracy}%` : "—"}</b></article>)}<div className="exam-pace"><p className="eyebrow">Exam pace</p><p>{averagePace ? averagePace <= 60 ? `You are averaging ${averagePace} seconds a question against a 60-second exam budget — comfortable.` : `You are averaging ${averagePace} seconds a question, which is ${averagePace - 60} seconds over the 60-second exam budget.` : "Answer a few questions to see your pace against the 60-second exam budget."}</p><Link className="button-link-muted" href="/cards">Review rule cards →</Link></div></section><section className="panel progress-session-log"><p className="eyebrow">Session log</p>{log.length ? log.map((item) => <article key={item.id} className="progress-log-row"><time>{new Date(item.at).toLocaleDateString(undefined, { day: "numeric", month: "short" })}</time><span>{item.label}</span><b className={item.tone}>{item.score}</b></article>) : <div className="empty-state"><h2>No sessions yet.</h2><p className="muted">Choose a course and answer your first question to begin your record.</p><Link className="button-link" href="/practice">Start practice</Link></div>}</section></div></>}
