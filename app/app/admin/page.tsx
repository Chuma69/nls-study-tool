...
export default function AdminPage() {
  const [tab, setTab] = useState<"users" | "experts">("users"); const [email, setEmail] = useState(""); const [invite, setInvite] = useState(""); const [items, setItems] = useState<Item[]>([]); const [users, setUsers] = useState<ActivityUser[]>([]); const [msg, setMsg] = useState("");
  async function load() {
  ...
  return <main><Link className="back-link" href="/">← Home</Link><p className="eyebrow">Admin</p><h1>Manage the tool.</h1>
    <nav className="admin-tabs" aria-label="Admin sections"><button className={tab === "users" ? "active" : ""} type="button" onClick={() => setTab("users")}>Users</button><button className={tab === "experts" ? "active" : ""} type="button" onClick={() => setTab("experts")}>Experts</button></nav>
    {tab === "users" && <>
