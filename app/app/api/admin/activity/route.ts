...
type AttemptRow = { user_id: number; answers: number; correct: number; distinct_q: number; last_answer_at: string | null; active_days: number };
type SessionRow = { user_id: number; sessions_count: number; total_seconds: number; last_active_at: string | null };
type CourseAttemptRow = { user_id: number; course: string; answers: number; correct: number };
type CourseTimeRow = { user_id: number; course: string; total_seconds: number };
...
export async function GET() {
  ...
  const [usersResult, attemptsResult, sessionsResult, courseAttemptsResult, courseTimesResult] = await Promise.all([
    ...
    `,
    sql`
      SELECT user_id, count(*)::int AS sessions_count, COALESCE(sum(total_seconds), 0)::int AS total_seconds, max(last_activity_at) AS last_active_at
    ...
    `,
    sql`
      SELECT user_id, course, COALESCE(sum(total_seconds), 0)::int AS total_seconds
      FROM practice_sessions WHERE course = ANY(${COURSE_IDS}) GROUP BY user_id, course
    `,
  ]);
  ...
  const courseTimes = courseTimesResult as unknown as CourseTimeRow[];
  ...
  for (const row of courseAttempts) { const c = courseFor(row.user_id, row.course); c.answers += row.answers; c.correct += row.correct; }
  for (const row of courseTimes) { const c = courseFor(row.user_id, row.course); c.total_seconds += row.total_seconds; }
  ...
  const enriched = users.map((user) => {
    ...
    const lastActive = [user.last_seen_at, a?.last_answer_at, s?.last_active_at]
      .filter(Boolean)
