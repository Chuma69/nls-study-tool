...
export async function POST(request: Request) {
  ...
  const body = await request.json() as { questionId?: number | string; category?: "typo" | "answer" | "other"; details?: string };
  const questionId = Number(body.questionId);
  if (!Number.isSafeInteger(questionId) || !["typo", "answer", "other"].includes(body.category ?? "")) return NextResponse.json({ error: "Choose the problem you found." }, { status: 400 });
