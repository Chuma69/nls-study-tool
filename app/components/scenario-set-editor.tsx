...
export function ScenarioSetEditor({ contextGroupId, onChanged }: { contextGroupId: string; onChanged?: () => void }) {
  ...
  async function load() {
    ...
    setQuestions(linked); setActiveId(linked[0]?.id ?? null); setScenario(linked[0]?.shared_context ?? "");
  }
