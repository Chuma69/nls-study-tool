...
export function ScenarioSetEditor({ contextGroupId, onChanged, triggerClassName, triggerChildren, openOnMount = false, initialQuestionId, hideTrigger = false }: { contextGroupId: string; onChanged?: () => void; triggerClassName?: string; triggerChildren?: ReactNode; openOnMount?: boolean; initialQuestionId?: number; hideTrigger?: boolean }) {
  ...
  const editor = open && typeof document !== "undefined" ? createPortal(<><div className="modal-backdrop scenario-set-backdrop" aria-hidden="true" /><section className="panel scenario-set-editor" role="dialog" aria-modal="true" aria-label="Edit full scenario set">
      ...
      {loading ? <p className="muted">Loading linked questions…</p> : error && !questions.length ? <p className="error">{error}</p> : <>
        <div className="scenario-set-details"><div><label>Course for this scenario set</label><select value={questions[0]?.course ?? ""} onChange={(event) => changeScenarioCourse(event.target.value)}>{Object.entries(COURSE_NAMES).map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select><p className="muted">Changing this applies the course to every linked question. Topics that do not belong to the new course will be cleared.</p></div><div><label>Edit the full case study or scenario</label><p className="muted">This shared text appears with every linked question. Save stores changes to the scenario, all questions, and their order.</p><textarea className="scenario-set-context" value={scenario} onChange={(event) => setScenario(event.target.value)} /></div></div>
